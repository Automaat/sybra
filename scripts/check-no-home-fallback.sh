...
#
# This is a literal-string grep, not an AST check — it looks for the ".sybra"
# suffix combined with a home-dir lookup in the same file, since that's the
# actual pattern that caused the incident. It intentionally does not flag
# every os.UserHomeDir()/homedir() call (e.g. resolving ~/.claude, ~/.codex,
# or generic "~/" expansion), only ones that also spell out ".sybra".
...
# Go: filepath.Join(<home-ish>, ".sybra") or string literal ".sybra" next to
# a UserHomeDir()-derived variable.
...
done < <(grep -rn '"\.sybra"' --include="*.go" . | grep -v '/vendor/' | sed 's#^\./##')

# TS/JS/Svelte: join(homedir(), '.sybra') or similar.
while IFS=: read -r file line _; do
  [[ -z "${file}" ]] && continue
  if ! is_allowlisted "${file}"; then
...
done < <(grep -rn "'\.sybra'" --include="*.ts" --include="*.js" --include="*.svelte" . | grep -v '/node_modules/' | grep -v '/bindings/' | sed 's#^\./##')
