...
func cmdSelfmonitorScan(jsonOut bool) int {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return fatal(jsonOut, "no selfmonitor report yet (run `synapse-cli selfmonitor investigate` for a one-shot pass, or start synapse with self_monitor.enabled=true)")
		}
