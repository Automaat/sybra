<script lang="ts">
  ...
  const testPlanReviewTasks = $derived(taskStore.byStatus('test-plan-review'))
  ...
  <div class="flex w-72 shrink-0 flex-col overflow-y-auto border-r border-surface-300 bg-surface-50 dark:border-surface-700 dark:bg-surface-900">
    <div class="border-b border-surface-300 px-4 py-3 dark:border-surface-700">
      <h3 class="text-sm font-semibold text-surface-700 dark:text-surface-300">
        Test Plan Review
        {#if testPlanReviewTasks.length > 0}
          <span class="ml-1.5 rounded-full bg-secondary-500 px-1.5 py-0.5 text-xs font-medium text-white">{testPlanReviewTasks.length}</span>
        {/if}
      </h3>
    </div>

    {#if testPlanReviewTasks.length === 0}
      <div class="p-4 text-sm text-surface-400 italic">No tasks pending test plan review</div>
    {:else}
      <ul class="flex flex-col gap-1 p-2">
        {#each testPlanReviewTasks as t}
    ...
    {#if !selectedTask}
      <div class="flex flex-1 items-center justify-center text-surface-400">
        <div class="text-center">
          ...
          <p class="text-sm">Select a task to review its test plan</p>
        </div>
      </div>
    {:else}
      <!-- Header -->
      ...
      <div class="flex-1 overflow-y-auto px-6 py-4">
        <div class="mb-3 flex items-center gap-2">
          ...
          <span class="text-xs font-medium text-surface-500">TEST_PLAN.md</span>
        ...
        <div class="flex items-start gap-3">
          ...
          <div class="flex shrink-0 flex-col gap-2">
            ...
            <button
              ...
              title={hasLiveAgent ? 'Send message to live test-plan agent' : 'No live test-plan agent'}
            >Send Message</button>
          </div>
        </div>
      </div>
