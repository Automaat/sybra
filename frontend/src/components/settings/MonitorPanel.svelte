...
>
  <ToggleField
    ...
    keyPath="supervision.monitor.enabled"
  ...
  {#if settings.monitor.enabled}
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
      <NumberField id="mon-interval" label="Interval (seconds)" keyPath="supervision.monitor.interval" min={60}
      ...
      <TextField id="mon-model" label="Model" placeholder="sonnet" keyPath="supervision.monitor.model"
        bind:value={settings.monitor.model}
        modified={m.model !== d.model}
        onreset={() => (settings.monitor.model = d.model)} />
      <NumberField id="mon-dispatch" label="Dispatch limit" keyPath="supervision.monitor.dispatch_limit" min={0}
      ...
      <NumberField id="mon-cooldown" label="Issue cooldown (min)" keyPath="supervision.monitor.issue_cooldown" min={0}
    ...
    <AdvancedDisclosure label="Detection thresholds">
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <NumberField id="mon-stuck" label="Stuck human (hours)" keyPath="supervision.monitor.stuck_human" min={0} step={0.5}
        ...
        <NumberField id="mon-lost" label="Lost agent (minutes)" keyPath="supervision.monitor.lost_agent" min={0}
        ...
        <NumberField id="mon-prgap" label="PR gap grace (minutes)" keyPath="supervision.monitor.pr_gap_grace" min={0}
        ...
        <NumberField id="mon-failrate" label="Failure-rate threshold" keyPath="supervision.monitor.failure_rate_threshold" min={0} max={1} step={0.05}
      ...
      <TextField id="mon-label" label="Issue label" keyPath="supervision.monitor.issue_label"
        bind:value={settings.monitor.issueLabel}
        modified={m.issueLabel !== d.issueLabel}
        onreset={() => (settings.monitor.issueLabel = d.issueLabel)} />
      <TextField id="mon-repo" label="Issue repo" placeholder="owner/repo" keyPath="supervision.monitor.issue_repo"
  ...
  <ToggleField
    ...
    keyPath="supervision.self_monitor.enabled"
  ...
  {#if settings.selfMonitor.enabled}
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
      <NumberField id="sm-interval" label="Interval (hours)" keyPath="supervision.self_monitor.interval" min={0} step={0.5}
      ...
      <NumberField id="sm-maxissues" label="Max issues / run" keyPath="supervision.self_monitor.max_issues_per_run" min={0}
    ...
    <ToggleField label="Dry run" description="Analyze and log but never file issues or take auto-actions"
      keyPath="supervision.self_monitor.dry_run"
