...
<Section title="Orchestrator" description="Dispatch and recovery cadence for the in-process monitor loop.">
  <AdvancedDisclosure label="Loop cadence">
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
      <NumberField id="orch-dispatch" label="Dispatch interval (seconds)" keyPath="workflow.orchestrator.dispatch_interval" min={1}
      ...
      <NumberField id="orch-maint" label="Maintenance interval (seconds)" keyPath="workflow.orchestrator.maintenance_interval" min={1}
  ...
  <AdvancedDisclosure label="Plan review">
    <ToggleField
      ...
      keyPath="workflow.orchestrator.auto_approve_plans_without_decisions"
