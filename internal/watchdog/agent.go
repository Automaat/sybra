...
// expectedStatus precondition when non-empty. Every call site here acts on a
// task with a live, registered watchdog-tracked agent — an invariant that
// only ever holds while the task is in-progress — so agent.go's own call
// sites (which act on the agent, not a freshly-read task) pass
// task.StatusInProgress; dwell.go/runrate.go, which already list-scanned and
// filtered the task, pass the status they observed.
...
func (w *Watchdog) applyVerdict(ctx context.Context, ag *agent.Agent, trigger string, verdict agent.InspectorVerdict) {
	...
	case "stop":
		...
		// budget trigger) remain immediate human-required escalations, unless
...
// names a concrete file:line finding for a fix-review agent to act on. The
// adversarial-review skill (internal/skills/data/adversarial-review.md) emits
// every finding with a "*Location:* `path:line`" line, so its presence is a
// cheap, reliable signal that the sidecar has somewhere concrete to point a
// retry at rather than being empty or purely prose.
func hasUnaddressedReviewFinding(codeReview string) bool {
