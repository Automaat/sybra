...
// test for Todoist-sourced tasks landing in Sybra with empty project_id,
...
func TestTodoistImport_SetsDefaultProjectID(t *testing.T) {
	...
	cfg := config.TodoistConfig{
		...
		APIToken:         "tok",
		ProjectID:        "tdProj",
		DefaultProjectID: "Automaat/sybra",
		PollSeconds:      60,
	}
	...
	if got.ProjectID != "Automaat/sybra" {
		t.Errorf("project_id = %q, want Automaat/sybra", got.ProjectID)
	}
