...
func (h *humanReviewHandler) buildPrompt(t task.Task, wctx *WorkScrubContext) string {
	...
	b.WriteString("- Before accepting a test-runner product_bug FAIL, check whether the task body has a later section that supersedes the wording the failure quotes (markers like \"supersed(es/ing)\", \"revised acceptance criteria\", \"design decision (superseding...)\", \"no longer applies\"). If the FAIL cites requirements text that a later section in the same body walks back, that FAIL is stale — re-verify the failure against the CURRENT acceptance criteria and the current repo state before counting it as a grounded defect.\n")
