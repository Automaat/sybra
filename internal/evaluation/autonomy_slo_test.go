...
func TestComputeAutonomySLOsTypedAndUnknownEvidence(t *testing.T) {
	...
	if got.ValidHumanEscalation.Success != 1 || got.ValidHumanEscalation.Known != 2 || got.ValidHumanEscalation.Unknown != 1 || got.MachineHumanRequired != 1 {
		t.Fatalf("human escalation metrics = %+v violations=%d", got.ValidHumanEscalation, got.MachineHumanRequired)
	}
	if got.RecoverySuccess.Rate != 1 || got.RepeatRepair.Rate != 1 || got.IncidentFanout.Max != 4 {
