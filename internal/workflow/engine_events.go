...
func (e *Engine) handleWatchdogRateLimitRetry(t *TaskInfo, step *Step) bool {
	return e.boundedRetry(t, step, boundedRetryPolicy{
		...
		armedClearIf: func(_ *Engine, t *TaskInfo, _ *Step, _ int) (expectedStatus, expectedReason string) {
			// The counter now represents a real retry attempt. Clear the watchdog
			// marker in the same write so a benign capacity park cannot make the
			// next ResumeStalled tick re-arm and burn this budget again — and so
			// a crash between the two can never leave the counter spent with the
			// marker still standing. The compare is atomic: a concurrent terminal
			// failure must win rather than being reopened from this stale retry
			// snapshot.
			return t.Status, t.StatusReason
		},
		onExhausted: func(e *Engine, t *TaskInfo, step *Step, attempts int) {
