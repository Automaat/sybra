...
func TestApplyTestVerdictCompletion_ClassifiesOutcomes(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:       "expected_behaviour_counts_as_evidence",
			status:     "completed",
			output:     `{"verdict":"FAIL"}`,
			bodySuffix: strings.ReplaceAll(groundedReport, "Expected:", "Expected behaviour:"),
			want:       testOutcomeProductBug,
			wantStatus: "completed",
		},
		{
		...
		{
			name:       "actual_behaviour_without_verbatim_output_is_missing_evidence",
			status:     "completed",
			output:     `{"verdict":"FAIL"}`,
			bodySuffix: "\n\n## Test Failures\n\nClassification: product_bug\n\nCommand run: go test ./internal/workflow\nActual behaviour: the workflow routed incorrectly.\nExpected behaviour: the workflow should reject weak evidence.\nCode evidence: internal/workflow/engine_steps_testroute.go:660\n",
			want:       testOutcomeMissingEvidence,
			wantStatus: "failed",
			wantTaint:  testProtocolMissingEvidence,
		},
		{
			name:       "unexpected_behaviour_does_not_satisfy_expected_evidence",
			status:     "completed",
			output:     `{"verdict":"FAIL"}`,
			bodySuffix: "\n\n## Test Failures\n\nClassification: product_bug\n\nCommand run: go test ./internal/workflow\nVerbatim output:\n```text\nFAIL\n```\n\nUnexpected behaviour: the workflow routed incorrectly.\nCode evidence: internal/workflow/engine_steps_testroute.go:663\n",
			want:       testOutcomeMissingEvidence,
			wantStatus: "failed",
			wantTaint:  testProtocolMissingEvidence,
		},
		{
			name:       "unexpected_behaviour_should_not_text_does_not_satisfy_expected_evidence",
			status:     "completed",
			output:     `{"verdict":"FAIL"}`,
			bodySuffix: "\n\n## Test Failures\n\nClassification: product_bug\n\nCommand run: go test ./internal/workflow\nVerbatim output:\n```text\nFAIL\n```\n\nUnexpected behaviour: the app should not return 500.\nCode evidence: internal/workflow/engine_steps_testroute.go:663\n",
			want:       testOutcomeMissingEvidence,
			wantStatus: "failed",
			wantTaint:  testProtocolMissingEvidence,
		},
