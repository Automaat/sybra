...
func TestBuiltinPRReview_AuthorizesVisibleReviewAction(t *testing.T) {
	...
	for _, stepID := range []string{"review_simple", "review_staff"} {
		...
		for _, want := range []string{
			...
			"NEVER submit an APPROVE review event",
		...
		for _, banned := range []string{
			...
			if strings.Contains(step.Config.Prompt, banned) {
				t.Fatalf("%s prompt still authorizes live aggregate review via %q:\n%s", stepID, banned, step.Config.Prompt)
			}
		}
		...
		if strings.Contains(step.Config.Prompt, "event=APPROVE") && !strings.Contains(step.Config.Prompt, "submit `event=APPROVE`") {
			t.Fatalf("%s prompt authorizes event=APPROVE somewhere unexpected:\n%s", stepID, step.Config.Prompt)
		}
	}
}
