...
func TestMetricsPipeline(t *testing.T) {
	...
	AgentStarted("claude", "headless")
	AgentCompleted(context.Background(), "ok", 2*time.Second)
	AgentCompleted(context.Background(), "error", 500*time.Millisecond)
	TaskCreated()
	TaskCreated()
	TaskUpdated()
	TaskDeleted()
	...
	MonitorTick(context.Background())
	MonitorAnomaly(context.Background(), "lost_agent")
	MonitorAnomaly(context.Background(), "pr_gap")
	OrchestratorTick(context.Background())
	...
	ProviderHealthFlip(context.Background(), "claude", false)
	ProviderHealthFlip(context.Background(), "claude", true)
	ProviderAuthFailure("claude")
	ProviderRateLimit("codex")
	AgentFailover("claude", "codex")
	AgentGated("claude", "logged_out")
