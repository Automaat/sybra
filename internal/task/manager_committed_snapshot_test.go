...
func TestRegressionAddRunActorBindsOwnCommittedSnapshot(t *testing.T) {
	...
	if _, err := second.Update(tk.ID, Update{CodeReviewVerdict: Ptr("NEEDS_FIXES")}); err != nil {
		t.Fatal(err)
	}
	...
	first.SetStatusTransitionHook(func(_, from, to, actor string, snapshot Task) {
		if snapshot.CodeReviewVerdict != "NEEDS_FIXES" {
			t.Errorf("committed primary verdict overwritten by sidecar hydration: %q", snapshot.CodeReviewVerdict)
		}
