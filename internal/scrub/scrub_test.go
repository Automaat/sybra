...
func TestScrub(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:       "lowercase jira-shaped text is not redacted",
			text:       "this mentions kag-1234 as ordinary lowercase text",
			blocklist:  nil,
			wantCount:  0,
			wantHas:    []string{"kag-1234"},
			wantNotHas: []string{Placeholder},
		},
		{
		...
		{
			name:       "github URL with lowercase numeric repo segment redacted whole",
			text:       "see https://github.com/work-org/secret-repo-2/pull/9 for context",
			blocklist:  nil,
			wantCount:  1,
			wantHas:    []string{Placeholder, "for context"},
			wantNotHas: []string{"work-org", "secret-repo-2", "/pull/9", "]]"},
		},
		{
		...
		{
			name:       "blocklist term inside placeholder does not rematch",
			text:       "task from work-org here",
			blocklist:  []string{"work-org", "act"},
			wantCount:  1,
			wantHas:    []string{"task from " + Placeholder + " here"},
			wantNotHas: []string{"work-org", "[red" + Placeholder + "ed]"},
		},
		{
			name:       "lowercase jira-shaped text still redacted by blocklist",
			text:       "this is blocked by kag-1234 over in tickets",
			blocklist:  []string{"KAG-1234"},
			wantCount:  1,
			wantHas:    []string{Placeholder, "over in tickets"},
			wantNotHas: []string{"kag-1234"},
		},
		...
		{
			name:       "unrelated hostname containing github is not redacted",
			text:       "see https://not-github-related.example.com/public/info for details",
			blocklist:  nil,
			wantCount:  0,
			wantHas:    []string{"https://not-github-related.example.com/public/info"},
			wantNotHas: []string{Placeholder},
		},
		{
