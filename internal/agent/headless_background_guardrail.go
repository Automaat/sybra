...
// down any live background bash task (`run_in_background`) at that point —
// Sybra's own process supervision never gets a chance to wait longer,
// because the child has already decided to exit. A command killed mid-write
// leaves the worktree silently corrupted rather than failing loudly: task
// 3aeabb65 traced a deterministic verify_checks failure to a `npm ci`
// backgrounded and then interrupted by end-of-turn, which left
...
// the background and ended anyway. The only reliable fix is telling the
// agent never to end its turn with a background task still running.
